...
@Service
public class CustomerOfferRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CustomerOfferRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.customeroffer.hosturl}")
	private String url;
	
	@Value("${spring.api.customeroffer.context-path}")
	private String contextPath;

	@Value("${spring.api.customeroffer.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CustomerOfferRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CustomerOfferRestProcessor.class.getName());
