...
@Service
public class SavingsAccountRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(SavingsAccountRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.savingsaccount.hosturl}")
	private String url;
	
	@Value("${spring.api.savingsaccount.context-path}")
	private String contextPath;

	@Value("${spring.api.savingsaccount.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  SavingsAccountRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ SavingsAccountRestProcessor.class.getName());
