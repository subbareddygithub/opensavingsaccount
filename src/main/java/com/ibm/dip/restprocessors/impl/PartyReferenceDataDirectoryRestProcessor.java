...
@Service
public class PartyReferenceDataDirectoryRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(PartyReferenceDataDirectoryRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.partyreferencedatadirectory.hosturl}")
	private String url;
	
	@Value("${spring.api.partyreferencedatadirectory.context-path}")
	private String contextPath;

	@Value("${spring.api.partyreferencedatadirectory.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  PartyReferenceDataDirectoryRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ PartyReferenceDataDirectoryRestProcessor.class.getName());
