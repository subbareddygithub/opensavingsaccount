...
@Service
public class CustomerProductsAndServicesRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CustomerProductsAndServicesRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.customerproductsandservices.hosturl}")
	private String url;
	
	@Value("${spring.api.customerproductsandservices.context-path}")
	private String contextPath;

	@Value("${spring.api.customerproductsandservices.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CustomerProductsAndServicesRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CustomerProductsAndServicesRestProcessor.class.getName());
