...
@Service
public class PaymentOrderRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(PaymentOrderRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.paymentorder.hosturl}")
	private String url;
	
	@Value("${spring.api.paymentorder.context-path}")
	private String contextPath;

	@Value("${spring.api.paymentorder.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  PaymentOrderRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ PaymentOrderRestProcessor.class.getName());
