...
@Service
public class FinancialAccountingRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(FinancialAccountingRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.financialaccounting.hosturl}")
	private String url;
	
	@Value("${spring.api.financialaccounting.context-path}")
	private String contextPath;

	@Value("${spring.api.financialaccounting.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  FinancialAccountingRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ FinancialAccountingRestProcessor.class.getName());
