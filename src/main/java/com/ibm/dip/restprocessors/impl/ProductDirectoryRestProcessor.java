...
@Service
public class ProductDirectoryRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(ProductDirectoryRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.productdirectory.hosturl}")
	private String url;
	
	@Value("${spring.api.productdirectory.context-path}")
	private String contextPath;

	@Value("${spring.api.productdirectory.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  ProductDirectoryRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ ProductDirectoryRestProcessor.class.getName());
