...
@Service
public class PartyLifecycleManagementRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(PartyLifecycleManagementRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.partylifecyclemanagement.hosturl}")
	private String url;
	
	@Value("${spring.api.partylifecyclemanagement.context-path}")
	private String contextPath;

	@Value("${spring.api.partylifecyclemanagement.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  PartyLifecycleManagementRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ PartyLifecycleManagementRestProcessor.class.getName());
