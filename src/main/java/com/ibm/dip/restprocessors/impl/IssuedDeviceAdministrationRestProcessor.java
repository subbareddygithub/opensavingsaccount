...
@Service
public class IssuedDeviceAdministrationRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(IssuedDeviceAdministrationRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.issueddeviceadministration.hosturl}")
	private String url;
	
	@Value("${spring.api.issueddeviceadministration.context-path}")
	private String contextPath;

	@Value("${spring.api.issueddeviceadministration.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  IssuedDeviceAdministrationRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ IssuedDeviceAdministrationRestProcessor.class.getName());
