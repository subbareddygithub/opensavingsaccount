...
@Service
public class CorrespondenceRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CorrespondenceRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.correspondence.hosturl}")
	private String url;
	
	@Value("${spring.api.correspondence.context-path}")
	private String contextPath;

	@Value("${spring.api.correspondence.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CorrespondenceRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CorrespondenceRestProcessor.class.getName());
