...
@Service
public class CustomerProductAndServiceEligibilityRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CustomerProductAndServiceEligibilityRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.customerproductandserviceeligibility.hosturl}")
	private String url;
	
	@Value("${spring.api.customerproductandserviceeligibility.context-path}")
	private String contextPath;

	@Value("${spring.api.customerproductandserviceeligibility.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CustomerProductAndServiceEligibilityRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CustomerProductAndServiceEligibilityRestProcessor.class.getName());
