...
@Service
public class GuidelineComplianceRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(GuidelineComplianceRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.guidelinecompliance.hosturl}")
	private String url;
	
	@Value("${spring.api.guidelinecompliance.context-path}")
	private String contextPath;

	@Value("${spring.api.guidelinecompliance.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  GuidelineComplianceRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ GuidelineComplianceRestProcessor.class.getName());
