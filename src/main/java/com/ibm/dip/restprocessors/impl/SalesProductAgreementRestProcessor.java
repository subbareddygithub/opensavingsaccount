...
@Service
public class SalesProductAgreementRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(SalesProductAgreementRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.salesproductagreement.hosturl}")
	private String url;
	
	@Value("${spring.api.salesproductagreement.context-path}")
	private String contextPath;

	@Value("${spring.api.salesproductagreement.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  SalesProductAgreementRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ SalesProductAgreementRestProcessor.class.getName());
